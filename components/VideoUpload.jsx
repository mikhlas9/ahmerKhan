...
import { uploadVideo, deleteImage, isFirebaseStorageUrl } from "@/lib/storage"

/**
 * VideoUpload Component
 * Handles video upload to Firebase Storage with preview and delete functionality
 * 
 * @param {string} value - Current video URL or path
 * @param {function} onChange - Callback when video changes (receives new URL or empty string)
 * @param {string} folder - Firebase Storage folder (e.g., 'homepage', 'videos')
...
export default function VideoUpload({ 
  ...
  const uniqueId = id || `video-upload-${folder}-${Math.random().toString(36).substr(2, 9)}`
  ...
  const handleFileSelect = async (e) => {
    ...
    if (file.size > 2 * 1024 * 1024 * 1024) {
      setError('Video size must be less than 2GB')
      return
    }
  ...
  const handleDelete = async () => {
    ...
    if (isFirebaseStorageUrl(value)) {
      const result = await deleteImage(value) // Reuse deleteImage function
      if (!result.success) {
        setError(result.error || 'Failed to delete video from storage')
        return
      }
    }

    // Clear the video (empty string means no video)
    setPreview("")
    onChange("")
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-3 items-center">
        ...
        >
          {uploading ? "Uploading..." : preview ? "Replace Video" : "Choose Video"}
        </label>
      ...
      <p className="text-xs text-gray-500">
        {preview 
          ? "Click 'Replace Video' to upload a new video, or 'Delete' to remove. You can also manually enter a path."
          : "Click 'Choose Video' to upload from your device, or manually enter a path (e.g., /videos/video.mp4)."
        }
      </p>
    </div>
  )
