 ...
 * @param {string} folder - The folder path in storage (e.g., 'homepage', 'videos')
 * @param {string} filename - Optional custom filename. If not provided, uses original filename with timestamp
 * @returns {Promise<{success: boolean, url?: string, error?: string}>}
 */
export async function uploadVideo(file, folder = 'videos', filename = null) {
  try {
    // Validate file type
    ...
    if (file.size > maxSize) {
      ...
      return { success: false, error: `Video size (${fileSizeGB}GB) must be less than 2GB` }
    }
