...
export async function createAward(awardData) {
  try {
    ...
    let newOrder
    if (awardData.order !== undefined && awardData.order >= 0) {
    ...
    existingAwards.forEach((award) => {
      const awardOrder = award.order !== undefined ? award.order : 999
      if (awardOrder >= 0 && awardOrder >= newOrder) {
    ...
    if (oldOrder !== newOrder) {
      ...
      if (newOrder < oldOrder) {
        ...
        allAwards.forEach((award) => {
          const awardOrder = award.order !== undefined ? award.order : 999
          if (award.id !== awardId && awardOrder >= 0 && awardOrder >= newOrder && awardOrder < oldOrder) {
            const otherAwardRef = doc(db, 'awards', award.id)
            batch.update(otherAwardRef, {
              order: awardOrder + 1,
              updatedAt: Timestamp.now()
        ...
        allAwards.forEach((award) => {
          const awardOrder = award.order !== undefined ? award.order : 999
          if (award.id !== awardId && awardOrder >= 0 && awardOrder > oldOrder && awardOrder <= newOrder) {
            const otherAwardRef = doc(db, 'awards', award.id)
            batch.update(otherAwardRef, {
              order: awardOrder - 1,
              updatedAt: Timestamp.now()
      ...
      // No order change, just update normally (but ensure order is at least 0)
      const dataToSave = {
        ...awardData,
        order: newOrder,
        updatedAt: Timestamp.now()
...
export async function deleteAward(awardId) {
  try {
    ...
    allAwards.forEach((award) => {
      const awardOrder = award.order !== undefined ? award.order : 999
      if (awardOrder >= 0 && awardOrder > deletedOrder) {
        const otherAwardRef = doc(db, 'awards', award.id)
        batch.update(otherAwardRef, {
          order: awardOrder - 1,
          updatedAt: Timestamp.now()
